...
public class PlotFFT extends JFrame {
    private static final long serialVersionUID = 6294689542092367723L;

    public PlotFFT(String title) {
        super(title);
        ...
        JFreeChart chart = ChartFactory.createScatterPlot(
                "Magniture VS frequency", "Frequency in Hz", "Magnitude in dB",
                dataset);
    ...
    private XYDataset createDataset() {
        ...
        SimpleReader in = new SimpleReader1L("FFTresult.txt");
        int mapIndex = 0;
        while (!in.atEOS()) {
        ...
        for (int j = 0; j < transformBins.size() / 2; j++) {
            series1.add(j * (44100.0 / 32768.0), transformBins.value(j));
        }
